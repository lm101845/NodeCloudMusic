...
# 转载的网易云音乐api文档，方便在线阅读，原文档地址打不开

## 原文档地址[网易云音乐api文档](https://link.juejin.cn/?target=https%3A%2F%2Fbinaryify.github.io%2FNeteaseCloudMusicApi%2F)

## 原api地址[网易云音乐api文档](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi)
...
[disoul/electron-cloud-music](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fdisoul%2Felectron-cloud-music)

[darknessomi/musicbox](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fdarknessomi%2Fmusicbox)

[sqaiyan/netmusic-node](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fsqaiyan%2Fnetmusic-node)
...
!>2018.10.14 更新到 3.0.0,使用了模块化机制,因为部分接口参数和 url 做了调整,如还不想升级到 3.0.0,请查看 [v2 的文档](https://link.juejin.cn/?target=http%3A%2F%2Fbinaryify.github.io%2FNeteaseCloudMusicApi%2F%23%2Fv2), [更新日志](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Fblob%2Fmaster%2FCHANGELOG.MD), [2.0+下载地址](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Freleases%2Ftag%2Fv2.20.5), 同时 2.0+ 将不再维护

## Docker 容器运行

> 注意: 在 docker 中运行的时候, 由于使用了 request 来发请求, 所以会检查几个 proxy 相关的环境变量(如下所列), 这些环境变量 会影响到 request 的代理, 详情请参考[request 的文档](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Frequest%2Frequest%23proxies), 如果这些环境变量 指向的代理不可用, 那么就会造成错误, 所以在使用 docker 的时候一定要注意这些环境变量. 不过, 要是你在 query 中加上了 proxy 参数, 那么环境变量会被覆盖, 就会用你通过 proxy 参数提供的代理了.
...
!> 由于接口做了缓存处理 ( 缓存 2 分钟,不缓存数据极容易引起网易服务器高频ip错误 , 可在 app.js 设置 , 可能会导致登陆后获取不到 cookie), **相同的 url** 会在两分钟内只向网易服务器发一次请求 , 如果遇到不需要缓 存结果的接口 , 可在请求 url 后面加一个时间戳参数使 url 不同 , 例子 : `/simi/playlist?id=347230&timestamp=1503019930000` (之所以加入缓存机制是因为项目早期没有缓存机制，很多 issues 都是报 IP高频，请按自己需求改造缓存中间件(app.js)，源码不复杂)

!> 如果是跨域请求 , 请在所有请求带上 `xhrFields: { withCredentials: true }` (axios 为 `withCredentials: true`)否则 可能会因为没带上 cookie 导致 301, 具体例子可看 `public/test.html`, 访问`http://localhost:3000/test.html`(默认端口的话) 例子使用 jQuery 和 axios
...
!> 由于网易限制,此项目在国外服务器或部分国内云服务上使用会受到限制,如 `460 cheating异常`,如需解决 , 可使用大陆服务器或者使用代理 , 感谢 [@hiyangguo](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fhiyangguo)提出的[解决方法](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Fissues%2F29%23issuecomment-298358438): 在 '/util/request.js' 的 'headers' 处增加 `X-Real-IP':'211.161.244.70' // 任意国内 IP` 即可解决
...
说明 : 登陆后调用此接口,使用`'Content-Type': 'multipart/form-data'`上传图片formData(name为'imgFile'),可更新头像(参考:[github.com/Binaryify/N…](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Fblob%2Fmaster%2Fpublic%2Favatar_update.html))
...
说明 : 登陆后调用此接口,使用`'Content-Type': 'multipart/form-data'`上传图片formData(name为'imgFile'),可更新歌单封面(参考:[github.com/Binaryify/N…](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Fblob%2Fmaster%2Fpublic%2Fplaylist_cover_update.html))
...
说明 : 调用此接口,传入歌单 id 可获取相关歌单(对应页面 [music.163.com/#/playlist?…](https://link.juejin.cn/?target=https%3A%2F%2Fmusic.163.com%2F%23%2Fplaylist%3Fid%3D1))
...
说明 : 歌单能看到歌单名字, 但看不到具体歌单内容 , 调用此接口 , 传入歌单 id, 可 以获取对应歌单内的所有的音乐(未登录状态只能获取不完整的歌单,登录后是完整的)，但是返回的trackIds是完整的，tracks 则是不完整的，可拿全部 trackIds 请求一次 `song/detail` 接口获取所有歌曲的详情 ([github.com/Binaryify/N…](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Fissues%2F452))
...
返回数据如下图 : ![歌单详情](https://raw.githubusercontent.com/Binaryify/NeteaseCloudMusicApi/master/static/%E6%AD%8C%E5%8D%95%E8%AF%A6%E6%83%85.png)

### 获取音乐 url

说明 : 使用歌单详情接口后 , 能得到的音乐的 id, 但不能得到的音乐 url, 调用此接口 , 传入的音乐 id( 可多个 , 用逗号隔开 ), 可以获取对应的音乐的 url( 不需要登录 )

> 注 : 部分用户反馈获取的 url 会 403,[hwaphon](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fhwaphon)找到的解决方案是当获取到音乐的 id 后，将 [music.163.com/song/media/…](https://link.juejin.cn/?target=https%3A%2F%2Fmusic.163.com%2Fsong%2Fmedia%2Fouter%2Furl%3Fid%3Did.mp3) 以 src 赋予 Audio 即可播放
...
返回数据如下图 : ![音乐 url](https://raw.githubusercontent.com/Binaryify/NeteaseCloudMusicApi/master/static/%E9%9F%B3%E4%B9%90%20url.png)
...
返回数据如下图 : ![搜索音乐](https://raw.githubusercontent.com/Binaryify/NeteaseCloudMusicApi/master/static/%E6%90%9C%E7%B4%A2.png)
...
**调用例子 :** `/comment/like?id=29178366&cid=12840183&t=1&type=0` 对应给 [music.163.com/#/song?id=2…](https://link.juejin.cn/?target=https%3A%2F%2Fmusic.163.com%2F%23%2Fsong%3Fid%3D29178366) 最热门的评论点赞
...
返回数据如下图 : ![获取歌曲详情](https://raw.githubusercontent.com/Binaryify/NeteaseCloudMusicApi/master/static/songDetail.png)
...
返回数据如下图 : ![获取专辑内容](https://raw.githubusercontent.com/Binaryify/NeteaseCloudMusicApi/master/static/%E4%B8%93%E8%BE%91.png)
...
说明: 请使用[歌单详情](https://juejin.cn/post/6860860407251369992#%E8%8E%B7%E5%8F%96%E6%AD%8C%E5%8D%95%E8%AF%A6%E6%83%85)接口,传入排行榜id获取排行榜详情数据(排行榜也是歌单的一种)
...
说明 : 登陆后调用此接口 , 传入`rid`, 可查看对应电台的电台节目以及对应的 id, 需要 注意的是这个接口返回的 mp3Url 已经无效 , 都为 null, 但是通过调用 `/song/url` 这 个接口 , 传入节目 id 仍然能获取到节目音频 , 如 `/song/url?id=478446370` 获取代 码时间的一个节目的音频
...
此文档由 [docsify](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FQingWei-Li%2Fdocsify%2F) 生成 docsify 是一个动 态生成文档网站的工具。不同于 GitBook、Hexo 的地方是它不会生成将 .md 转成 .html 文件，所有转换工作都是在运行时进行。

## License

[The MIT License (MIT)](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FBinaryify%2FNeteaseCloudMusicApi%2Fblob%2Fmaster%2FLICENSE)